 ...
 */
router.get("/salary-insights", async (req, res): Promise<void> => {
  const parsed = Query.safeParse(req.query);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.message });
    return;
  }
  const { jobId, institutionId } = parsed.data;
  ...
  const conditions = [
    ...
    sql`LOWER(${jobsTable.title}) = LOWER(${anchor.title})`,
  ];
