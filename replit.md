...
- **Email layer** (`artifacts/api-server/src/lib/email.ts`): `sendAuthLinkEmail({ to, fullName, linkPath, kind: "setup" | "reset", origin, logger })` is the single entry point. Today it logs the absolute URL and returns `{ sent: false, reason: "email-not-configured" }` so the admin onboarding success card and forgot-password page can fall back gracefully (the admin UI shows the copyable link only when `emailSent === false`). When the Resend integration is connected, swap the body of `sendAuthLinkEmail` to call Resend and return `{ sent: true, provider: "resend" }`.
